...
class Restaurant {
    ...
    public String toString() {
        return "The " + this.getName() + " Restaurant:\n" + "** Open from "
                + this.getOpeningTime().toString() + " to " + this.getClosingTime().toString() + " **";
    }

    public String itemsInfo() {
        String menuStr = "Menu at the " + this.getName() + " Restaurant:\n";
        for (int i = 0; i < this.getMenu().length; i++) {
            menuStr += this.getMenu()[i].toString();
        }
        return menuStr;
    }
}
