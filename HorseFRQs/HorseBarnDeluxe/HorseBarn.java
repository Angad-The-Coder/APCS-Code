...
public class HorseBarn {
    ...
    /**
     * Given a possible index within spaces, ind, checkBadIndex determines whether or not that index
     * is within the bounds of valid indices one could use to access elements in spaces and, if it
     * isn't, it throws an IndexOutOfBoundsException stating the value of the invalid index and then
     * the length of spaces, which would help the user by showing them why the index was invalid.
     * 
    ...
    public void checkBadIndex(int ind) {
        if (ind < 0 || ind >= this.spaces.length)
            throw new IndexOutOfBoundsException(
                    "Bad index: " + ind + " length: " + this.spaces.length);
    }
    ...
    /**
     * Returns the index of the space that contains the horse with the specified name. Precondition:
    ...
    /**
     * removes an array of the Horse Objects in spaces whose birth happened before the specified
     * Date. If no Horses were born before the Date, then null is returned. The returned array
     * should be just large enough to hold the removed Horses Here is a blurb from the Date class
     * compareTo method returns 0 if the argument Date is equal to this Date; a value less than 0 if
     * this Date is before the Date argument; and a value greater than 0 if this Date is after the
     * Date argument.
     */
