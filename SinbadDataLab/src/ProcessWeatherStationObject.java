 ...
 * have as many constructors as they need.  The only requirement is that the parameter lists cannot
 * be the same (because the compiler would not know which to call).  Update the toString method
 * Then, provide getters so that the Observations can be compared based on the new attributes.
 */
