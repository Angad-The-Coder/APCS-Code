...
public class ArrayTester {
    ...
    /**
     * prompt the user for the number of elements, the min element and max element to create. Then
     * create an array with that number of elements randomly ranging from min to max value. Assign
     * that array to nums (private instance field)
     */
     ...
     * 
     * @return an array with the modes. If there is only one mode, the array will have length 1
     */
    ...
    /**
     * find and return the median of the elements in nums Use the following algorithm: 1. starting
     * with min, count how many times it occurs. 2. If at any time you reach the halfway point of
     * the array, you have * found the median (middle number) 3. In the case of an array with an
     * even number of elements, then you will return the mean of the two middle elements.
     */
    private double findMedian() {
        ...
        if (sortedNums.length % 2 == 0) {
            return (double) (sortedNums[midIdx - 1] + sortedNums[midIdx]) / 2;
        }
    ...
    /**
     * Returns the standard deviation of the data set Use the following algorithm: 1. find the mean
     * 2. for each element - a) find the difference between the element and the mean b) square the
     * difference c) add the square to a running total 3. Take the running total of squares of
     * differences from the mean and divide by number of elements 4. Take the square root of that
     * number, that is the standard deviation. So, return it!
     */
    ...
    /**
     * Returns the percent of data within the specified number of standard deviations from the mean
     * of the data set Use the following algorithm: 1. find the mean 2. find the standard deviation
     * 3. calculate how far away an element has to be to be at least the specified number of
     * standards of deviation 4. for each element - a) check to see if it is within the specified
     * number of deviations. If so, count it.
     * 
     * 5. return the percent (num within stds of dev / size of array * 100)
     */
    private double percentWithinStdDev(double devs) {
        ...
        for (int i = 0; i < nums.length; i++) {
            if (nums[i] > (mean - devs * stdDev) && nums[i] < (mean + devs * stdDev))
                numWithinStdDev++;
        }
